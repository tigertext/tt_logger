...
exports.initLog = function(name, logLevel_, popcornHost, popcornPort) {
  ...
  if(index > 0) serviceName = serviceName.slice(index+1);
  winston.add(winston.transports.File, { filename: name, maxsize: 5000000, maxFiles: 10});
  winston.add(winston.transports.Popcorn , {level: 'info', service: serviceName, host: popcornHost, port: popcornPort});
